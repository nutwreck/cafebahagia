				...
				<div class='col-sm-9'>
					...
					<table class='table table-bordered' id='TabelTransaksi'>
						<thead>
							<tr>
								...
								<th style='width:210px;'>Kode Produk</th>
								<th>Nama Produk</th>
					...
					<div class='row'>
						<div class='col-sm-7'>
							...
							<br />
							<p><i class='fa fa-keyboard-o fa-fw'></i> <b>Bantuan Shortcut Keyboard : </b></p>
							<div class='row'>
...
<script>
	...
	function BarisBaru() {
		...
		Baris += "<td>" + Nomor + "</td>";
		Baris += "<td>";
		Baris += "<input type='text' class='form-control' name='kode_produk[]' id='pencarian_kode' placeholder='Ketik Kode / Nama Produk'>";
	...
	function CetakStruk() {
		if ($('#TotalBayarHidden').val() > 0) {
			...
			$('.modal-dialog').addClass('modal-sm');
			$('#ModalHeader').html('Oops !');
			$('#ModalContent').html('Harap pilih produk terlebih dahulu');
			$('#ModalFooter').html("<button type='button' class='btn btn-primary' data-dismiss='modal' autofocus>Ok</button>");
			$('#ModalGue').modal('show');
