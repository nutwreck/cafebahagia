				...
				if( ! empty($_POST['kode_produk']))
				{
					...
					if($total > 0)
					{
						...
						foreach($_POST['kode_produk'] as $d)
						{
							if( ! empty($d))
							{
								$this->form_validation->set_rules('kode_produk['.$no.']','Kode Produk #'.($no + 1), 'trim|required|max_length[40]|callback_cek_kode_produk[kode_produk['.$no.']]');
						...
						$this->form_validation->set_message('cek_kode_produk', '%s tidak ditemukan');
					...
					{
						$this->query_error("Harap masukan minimal 1 kode produk !");
					}
				}
				else
				{
