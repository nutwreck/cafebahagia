...
class M_penjualan_master extends CI_Model
{
	function insert_master($nomor_nota, $tanggal, $id_kasir, $id_pelanggan, $bayar, $grand_total, $catatan)
	{
		...
		return $this->db->insert('penjualan_master', $dt);
	}
	...
	function fetch_data_penjualan($like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL)
	{
		$sql = "
			...
			FROM 
				`penjualan_master` AS a 
				LEFT JOIN `pelanggan` AS b ON a.`id_pelanggan` = b.`id_pelanggan` 
				LEFT JOIN `user` AS c ON a.`id_user` = c.`id_user` 
	...
	function hapus_transaksi($id_penjualan, $reverse_stok)
	{
		if($reverse_stok == 'yes'){
			$loop = $this->db
				->select('id_produk, jumlah_beli')
				->where('id_penjualan_m', $id_penjualan)
				->get('penjualan_detail');

			foreach($loop->result() as $b)
			{
				$sql = "
					UPDATE `produk` SET `total_stok` = `total_stok` + ".$b->jumlah_beli." 
					WHERE `id_produk` = '".$b->id_produk."' 
				";
		...
		$this->db->where('id_penjualan_m', $id_penjualan)->delete('penjualan_detail');
		return $this->db
			->where('id_penjualan_m', $id_penjualan)
			->delete('penjualan_master');
	}
	...
	{
		return $this->db->select('nomor_nota')->where('nomor_nota', $nota)->limit(1)->get('penjualan_master');
	}
}